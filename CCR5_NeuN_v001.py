...
for condition_folder in os.listdir(root_path):
    ...
    for round_folder in os.listdir(condition_path):
        ...
        for files in os.listdir(cells_path):
            ...
            whole_ineuron_intensity = np.mean(CCR5_image)
            ...
            neuron_rois = readRoi(os.path.join(cells_path, str(file_header + "-NeuN_ROI.zip")))
            print("neuron rois: ", len(neuron_rois))
            CCR5_rois = readRoi(os.path.join(cells_path, str(file_header + "-CCR5_ROI.zip")))
            print("CCR5 rois: ", len(CCR5_rois))
            
            #calculate the area of the neuron
            # Create individual polygons for each ROI
            neuron_polygons = [Polygon(roi) for roi in neuron_rois]   
            
            neuron_area = 0
            for neuron in neuron_polygons:
                neuron_area += neuron.area * (resolution*resolution)
            ...
            CCR5_in_neuron_count = 0
            CCR5_in_neuron_area = 0
            ...
            neuron_intensities_CCR5 = []
            
            # Check which CCR5 ROIs are inside the dendrites and accumulate the area
            for neuron_polygon in neuron_polygons:
                # Get intensity values inside the ROI
                roi_intensity_values = get_intensity_values(neuron_polygon, CCR5_image)
                neuron_intensities_CCR5.append(np.mean(roi_intensity_values))
                
                for CCR5 in filtered_CCR5_polygons:
                    try:
                        if neuron_polygon.contains(CCR5):
                            CCR5_in_neuron_count += 1
                            CCR5_in_neuron_area += CCR5.area
                            
                    except:
                        print(f'In one of the neurons of {file_header} is no hyperphos punct.')
                        
            
            # Multiply areas by resolution if necessary (for scaling units)
            CCR5_in_neuron_area *= (resolution * resolution)
            
            
            # fill the dicitionary for saving
            data['file'].append(file_header)
            data['area_neuron'].append(neuron_area)
            data['CCR5_area_neuron'].append(CCR5_in_neuron_area/neuron_area)
            data['CCR5_puncta_neuron'].append(CCR5_in_neuron_count)
            data['CCR5_puncta_neuron_norm'].append(CCR5_in_neuron_count/neuron_area)
            ...
            data['total_mean_int'].append(whole_ineuron_intensity)
            data['mean_int_neuron'].append(np.mean(neuron_intensities_CCR5))
            
            data['total_IntDen'].append(whole_ineuron_intensity*area_img)
            data['IntDen_neuron'].append(np.mean(neuron_intensities_CCR5)*neuron_area)
    ...
    df_data.to_csv(results_path_condition + fr'/CCR5_neuron_data_{condition_folder}.csv', index=False)
