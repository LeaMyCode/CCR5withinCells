...
for (f = 0; f<fileList.length; f++){
		...
		if (channels == 3) {
			...
			run("Analyze Particles...", "size=30-500 add");
	    	roiManager('select', "*");
			...
			//Make copy of NeuN to compare original and thresholded one
			selectWindow("NeuN");
			run("Duplicate...", "title=NeuN_copy");
			
			//NeuN channel roi extraction
			selectWindow("NeuN");
			run("Enhance Contrast", "saturated=0.35");
			run("Apply LUT"); 
	    	...
	    	roiManager('select', "*");
			roiManager("Save", output_path + title+ "-NeuN_ROI.zip");
			roiManager("reset");
			
			//Save thresholded NeuN
			selectWindow("NeuN");
			saveAs("PNG", output_path + title + "-NeuN_threshold.png");
			run("Close");
